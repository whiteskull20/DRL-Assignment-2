...
class Connect6Game:
    def __init__(self, size=19):
        ...
        print(sorted(self.approximator.weights.items(),key=lambda x:x[1],reverse=True)[-20:],file=sys.stderr)
        self.mcts = MCTS( self.approximator, iterations=10, simulation_batch=1, rollout_depth=2, exploration_constant=1.41, gamma=0.999)
        self.remain_move = 1
    ...
    def generate_move(self, color):
        ...
        print("Current estimate =",estimate,self.turn,file=sys.stderr)
        if self.game_over:
            print("? Game over")
            return
